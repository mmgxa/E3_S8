...
def convert(cfg: DictConfig) -> Tuple[dict, dict]:
    ...
    # scripted_model = torch.jit.script(model.to("cpu").model.eval())
    path = f"{out_path}/model_vit.script.pt"
    torch.jit.save(scripted_model, path)
    log.info("Successfully Scripted Model...")

    log.info("Tracing Model...")
    traced_model = model.to("cpu").to_torchscript(method="trace", example_inputs=(torch.randn(1, 3, 32, 32)))
    path = f"{out_path}/model_vit.trace.pt"
